...
Corrective official-web search can use the unified Bailian credential with `OFFICIAL_WEB_ALLOWED_DOMAINS`, or a dedicated provider configured by `OFFICIAL_WEB_SEARCH_URL` and `OFFICIAL_WEB_SEARCH_API_KEY`. When unavailable, CampusFlow reports the tool as unavailable and continues with local Hybrid RAG; it never reports fake external results. See `THIRD_PARTY_NOTICES.md` for reference-project attribution and reuse boundaries.
