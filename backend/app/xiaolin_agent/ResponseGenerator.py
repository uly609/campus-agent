...
9. 只有 `data_mode: verified_official` 的资料可以称为学校官方来源；回答时应给出其标题和URL，有核验日期时再给出日期。`excerpt_kind: search_summary` 表示百炼基于官网搜索生成的摘要，不得把摘要描述成官网原文。`live_external: true` 只能称为实时外部数据。
